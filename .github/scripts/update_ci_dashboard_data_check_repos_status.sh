...
# - "push", "triage" and "pull": Access rights to the keyple-website repo differ when owner is eclipse.

github_hash=`curl --request GET \
          --url https://api.github.com/repos/eclipse/$repository_name \
          --header "authorization: Bearer $token" \
          --header "content-type: application/json" | grep -v -e "pushed_at" -e "size" -e "push" -e "triage" -e "pull" | md5sum | cut -d ' ' -f 1`
